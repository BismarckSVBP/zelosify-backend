...
/**
 * Verify TOTP (Step 2 of login flow)
 * - Checks temp token & TOTP
 * - Exchanges refresh token for new access/refresh tokens
 * - Updates DB + sets cookies
 */
export const verifyTOTP = async (
  req: Request,
  res: Response
  ...
  try {
    ...
    // 2. Decode & verify JWT temp token
    let decoded: JwtPayload;
    try {
      decoded = jwt.verify(tempToken, process.env.JWT_SECRET!) as JwtPayload;
    } catch (error) {
    ...
    // 5. Exchange refresh token with Keycloak for new tokens
    const clientSecret = await getKeycloakClientSecret();
